...
- Treat curated third-party skills as advisory. Project requirements, security
  policy, and documented Cloudflare architecture always take precedence.
...
- Deploy HTTP applications only as Cloudflare Workers services.
- Use Supabase for PostgreSQL and Secret Manager for database credentials.
...
- Implement scheduled or finite background work as Cloudflare Cron Triggers or Queue consumers invoked by
  Cron Triggers. Do not deploy Cloudflare Pages Functions as a separate stack.
- Deploy immutable Worker versions from the commit that passed Quality, using a scoped Cloudflare API token held as a repository secret.

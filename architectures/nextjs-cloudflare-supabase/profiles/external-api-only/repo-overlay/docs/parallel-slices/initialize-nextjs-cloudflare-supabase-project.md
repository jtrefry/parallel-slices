# Initialize a Next.js Cloudflare external-API project

Use this procedure only when `.parallel-slices/architecture.json` selects
`nextjs-cloudflare-supabase` with the `external-api-only` profile. This profile builds a
...
service, SQL scanner, schema-migration framework, Supabase configuration, or
...
- Cloudflare Workers service and job requirements;
...
- `docs/project/cloudflare-operations.md`; and

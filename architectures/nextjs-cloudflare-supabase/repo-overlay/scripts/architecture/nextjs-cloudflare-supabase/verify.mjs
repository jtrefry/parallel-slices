...
export function inspectArchitecture(root, options = {}) {
  ...
  if (!["postgres", "external-api-only"].includes(profile)) {
    fail(`unsupported nextjs-cloudflare-supabase profile: ${profile}`);
  }
  const packagePath = join(root, "package.json");
  if (!existsSync(packagePath)) {
    fail(
      "the Next.js Cloudflare PostgreSQL architecture requires a root package.json",
    );
  }
  if (
    ...
    fail(
      "the Next.js Cloudflare PostgreSQL architecture requires turbo.json or turbo.jsonc",
    );
  }
  const rootPackage = readJsonFile(packagePath, "package.json");
  if (!dependencyVersion(rootPackage, "turbo")) {
    fail(
      "the Next.js Cloudflare PostgreSQL architecture requires turbo in the root package",
    );
  }
  const manifests = repositoryPackageManifests(root);
  if (!manifests.some(([, manifest]) => dependencyVersion(manifest, "next"))) {
    fail(
      "the Next.js Cloudflare PostgreSQL architecture requires at least one Next.js package",
    );
  }
  ...
  return {
    architecture: "nextjs-cloudflare-supabase",
...
if (isMain) {
  try {
    ...
    console.error(
      `NEXTJS Cloudflare POSTGRES ARCHITECTURE FAILED: ${error.message}`,
    );
    process.exitCode = 1;
  }
}

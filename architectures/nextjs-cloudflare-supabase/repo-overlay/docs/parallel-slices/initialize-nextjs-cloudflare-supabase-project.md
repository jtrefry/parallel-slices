# Initialize a Next.js Cloudflare PostgreSQL project
...
branch, open a pull request, deploy, provision Cloudflare resources, access production
...
requirements and Cloudflare architecture.
...
6. Scheduled or finite background work that must run as Cloudflare Cron Triggers or Queue consumers through
   Cron Triggers.
...
10. Environments, Cloudflare organization and project ownership, domain and DNS
...
- PostgreSQL as the application database, with Supabase for production unless
...
- Docker Compose for a disposable local PostgreSQL, and `wrangler dev` for a local Worker;
- Cloudflare Workers, Queues, Cron Triggers, Hyperdrive, and Supabase PostgreSQL,
  Worker secrets, a scoped deployment API token, Hyperdrive bindings, and no
  Functions;
...
- `docs/project/cloudflare-operations.md`
...
- Cloudflare Workers, Supabase, job, scheduler, IAM, secrets, monitoring, rollback, and
  cost impact;
...
## Independent Product Plan review

When `.parallel-slices/review.json` enables multi-agent review, the plan must
survive independent review **before** the developer is asked to approve it:

```bash
node scripts/parallel-slices/review-plan.mjs --plan docs/plans/<plan>.md
```

Every configured reviewer reads the plan and the repository independently, and
all must approve. They look for what prose hides: acceptance evidence no
implementation could produce, a requirement with no observable evidence or no
possible owner, requirements that contradict each other or a stated non-goal, a
conflict with the root instructions or the installed architecture, a lifecycle
...
perfectly well; the contradiction otherwise surfaces after compilation, after
review of the compiled map, and after a person has already approved it.

## Product Plan approval checkpoint

Present the reviewers' verdicts alongside:
...
Use the approved Product Plan as source and the installed `nextjs-cloudflare-supabase`
...
data, authentication, integrations, Cloudflare definitions, and the first product
...
The installed Next.js/Cloudflare default is `throughput-balanced` because its full

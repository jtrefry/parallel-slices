# `nextjs-cloudflare-supabase` architecture package

This is the bundled, versioned architecture for Next.js applications deployed
to Cloudflare. It owns every Next.js, Turborepo, Mantine, Trivy, and Cloudflare
assumption installed by Parallel Slices, plus the data-layer contracts
...
- `postgres` is the default and includes PostgreSQL, ordered migrations, SQL
  security, a PostgreSQL CI service, and Supabase delivery configuration.
- `external-api-only` uses external APIs and includes no application database,
  ...
  service, or Supabase delivery configuration.
...
```bash
npm run bootstrap -- \
  --config examples/create/nextjs-cloudflare-supabase.json \
  /absolute/path/to/postgres-project

npm run bootstrap -- \
  --config examples/create/nextjs-cloudflare-external-api-only.json \

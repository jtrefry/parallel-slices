...
async function executeReviewLoop(context) {
  ...
  // Exactly one independent pass. Reviewers do not see each other's findings
  // and do not negotiate; every one must approve. Anything less returns the
  // findings to the controller, which fixes the map and runs review again.
  {
    const roundNumber = 1;
    const round = {
      number: roundNumber,
    ...
    for (const reviewer of context.reviewers) {
      ...
      console.log(
        `  ${reviewer.id}: ${invocation.response.verdict}, ${invocation.response.findings.length} findings`,
      );
    ...
    if (round.consensus.approved) {
      terminalAttempt(
        context.attempt,
        "approved",
        `All ${reviewerIds.length} configured reviewers approved independently with no critical or high findings.`,
        context.now(),
      );
      writeReviewLedger(context.root, context.paths, context.ledger);
  ...
  const unresolved = context.attempt.findings.map((finding) => finding.id);
  terminalAttempt(
    context.attempt,
    "changes_requested",
    unresolved.length
      ? `At least one reviewer requested changes. Findings: ${unresolved.join(", ")}.`
...
export async function runMultiAgentReview(options) {
  ...
  try {
    ...
    try {
      ...
      throw new Error(
        `review input was not sent to any reviewer: ${error.message}`,
        { cause: error },
      );
    }

...
function ensureSafeDirectory(root, target) {
  ...
  for (const segment of pathFromRoot.split(sep).filter(Boolean)) {
    ...
    if (!metadata.isDirectory() || metadata.isSymbolicLink()) {
      fail(`refusing unsafe runtime directory: ${relative(rootReal, current)}`);
    }
  }
  return absolute;
}
...
function runtimeLockIsStale(absolute, path) {
  let owner;
  try {
    owner = JSON.parse(readFileSync(absolute, "utf8"));
  } catch (error) {
    if (error.code === "ENOENT") return true;
    fail(
      `runtime lock file is unreadable: ${path}; verify its owner stopped, then remove it`,
    );
  }
  if (!Number.isInteger(owner?.pid) || owner.pid <= 0) {
    fail(
      `runtime lock file has no valid owner pid: ${path}; verify its owner stopped, then remove it`,
    );
  }
  try {
...
function releaseRuntimeLock(absolute, token) {
  let owner;
  try {
    owner = JSON.parse(readFileSync(absolute, "utf8"));
  } catch {
...
function withRuntimeLock(root, path, operation) {
  ...
  const acquire = () => {
    ...
    try {
      ...
      writeFileSync(
        descriptor,
        `${JSON.stringify({ pid: process.pid, startedAt: nowIso(), token })}\n`,
      );
      fsyncSync(descriptor);
    } finally {
      if (descriptor !== undefined) closeSync(descriptor);
  ...
  if (!acquired && runtimeLockIsStale(absolute, path)) {
    try {
      unlinkSync(absolute);
    } catch (error) {
      if (error.code !== "ENOENT") throw error;
  ...
  try {
    ...
    releaseRuntimeLock(absolute, token);
  }
}
...
export function claimIntegrationAttempt(root, workerId, goalBaseCommit) {
  ...
  return withRuntimeLock(root, indexLockPath(state.runId), () =>
    withRuntimeLock(root, `${tracking.integrationPath}.lock`, () => {
      ...
      if (
        ...
        return { ...current.integration, alreadyClaimed: true };
      }

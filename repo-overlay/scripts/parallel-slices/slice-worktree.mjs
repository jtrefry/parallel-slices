...
export function applySliceCandidate(root, workerId) {
  ...
  if (
    ...
    goalBaseCommit = goalHead;
    const claimed = claimIntegrationAttempt(root, workerId, goalBaseCommit);
    if (claimed.alreadyClaimed) {
      fail(
        `slice ${verified.metadata.slice} integration was already claimed by another process; verify it stopped, then rerun apply to resume`,
      );
    }
  }
  try {

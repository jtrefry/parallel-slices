...
function parseArgs(argv) {
  ...
  if (
    !["validate", "sets", "analyze", "diagram", "ready"].includes(command) ||
    !options.plan
  ) {
    fail(
      "usage: slice-graph.mjs validate|sets|analyze|diagram --plan <plan> | ready --plan <plan> --state <state>",
    );
  }
...
// Render the compiled execution map as a Mermaid DAG plus the two numbers a
// human actually wants: how many slices run at once, and how many waves it
// takes. Generated from the committed manifests so the diagram cannot drift
// from the map it describes.
export function renderExecutionDiagram(manifests) {
  const analysis = analyzeExecutionGraph(manifests);
  const sets = computeExecutionSets(manifests);
  const bySlice = new Map(manifests.map((m) => [m.slice, m]));
  const waveOf = new Map();
  sets.forEach((wave, index) =>
    wave.forEach((slice) => waveOf.set(slice, index + 1)),
  );
  const nodeId = (slice) => `S${slice.replace(/[^A-Za-z0-9]/g, "_")}`;
  const lines = ["flowchart LR"];
  sets.forEach((wave, index) => {
    lines.push(`  subgraph W${index + 1}["Wave ${index + 1}"]`);
    lines.push("    direction TB");
    for (const slice of wave) {
      const manifest = bySlice.get(slice);
      // Conventional commit scope is the shortest honest name a slice has.
      const scope = /^[a-z]+\(([^)]+)\)/.exec(manifest?.commit || "")?.[1];
      const label = scope ? `${slice} ${scope}` : slice;
      lines.push(`    ${nodeId(slice)}["${label}"]`);
    }
    lines.push("  end");
  });
  for (const edge of analysis.dependencyEdges) {
    lines.push(`  ${nodeId(edge.dependsOn)} --> ${nodeId(edge.slice)}`);
  }
  return {
    mermaid: lines.join("\n"),
    sliceCount: analysis.sliceCount,
    waveCount: analysis.executionSetCount,
    maxParallelWidth: analysis.maxParallelWidth,
    fullySerial: analysis.fullySerial,
    waves: sets.map((wave, index) => ({
      wave: index + 1,
      slices: wave,
      width: wave.length,
    })),
...
function runCli(argv) {
  ...
  } else if (options.command === "diagram") {
    const diagram = renderExecutionDiagram(manifests);
    console.log("```mermaid");
    console.log(diagram.mermaid);
    console.log("```");
    console.log("");
    console.log(
      `${diagram.sliceCount} slices in ${diagram.waveCount} waves; at most ${diagram.maxParallelWidth} run at once.`,
    );
    for (const wave of diagram.waves) {
      console.log(
        `  wave ${wave.wave}: ${wave.width} parallel (${wave.slices.join(", ")})`,
      );
    }
  } else {

...
export function evaluateHookInstallation(hooksPath, missingFiles) {
  if (hooksPath !== ".husky/_") {
    return {
      ok: false,
      message: `core.hooksPath is ${hooksPath || "not configured"}; run the package manager install so Husky activates .husky/_ and the local quality gates`,
    };
  }
  if (missingFiles.length) {
    return {
      ok: false,
      message: `Husky hook files are missing: ${missingFiles.join(", ")}; run the package manager install to regenerate them`,
    };
  }
  return { ok: true, message: "Husky hooks configured at .husky/_" };
}
...
export function runDoctor(options = {}) {
  ...
  const hookProbe = commandResult(
    "git",
    ["config", "--get", "core.hooksPath"],
    root,
  );
  const hookInstallation = evaluateHookInstallation(
    hookProbe.status === 0 ? hookProbe.stdout.trim() : "",
    [
      ".husky/pre-commit",
      ".husky/pre-push",
      ".husky/_/pre-commit",
  ...
  if (hookInstallation.ok) ok(hookInstallation.message);
  else warn(hookInstallation.message);

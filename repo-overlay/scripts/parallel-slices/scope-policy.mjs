...
export const controlPlanePrefixes = Object.freeze([
  ".parallel-slices/",
  ".husky/",
  ".github/",
...
export function findControlPlanePattern(patterns) {
  return patterns.find((pattern) =>
    controlPlanePrefixes.some((prefix) =>
      patternsMayOverlap(pattern, `${prefix}**`),
    ),
  );
}
...
export function validateManifest(manifest, scopeFile, root, config) {
  ...
  if (manifest.version === "1") {
    ...
    const controlPattern = findControlPlanePattern(manifest.allow);
    if (controlPattern) {
      fail(
        `worker allow patterns cannot cover the orchestration control plane: ${controlPattern}`,
      );
    }

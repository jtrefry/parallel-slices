...
import { primaryRepositoryRoot } from "./run-tracking.mjs";
...
function requireStagedMatchesWorktree(root, paths) {
  for (const path of paths) {
    if (git(["diff", "--name-only", "--", path], root)) {
      throw new Error(
        `staged planning contract does not match the working tree: ${path}`,
      );
    }
  }
}
...
function validateCompiledExecutionCommit(root, entries, config) {
  ...
  const manifestPaths = entries
    .filter(
      (entry) =>
        ...
        isExecutableManifest(entry.path),
    )
  ...
  const stagedStatePaths = entries
    .filter(
      (entry) =>
        (entry.status === "A" || entry.status === "M") &&
        isRunState(entry.path),
    )
    .map((entry) => entry.path);
  if (stagedStatePaths.length !== 1 || stagedStatePaths[0] !== statePath) {
    throw new Error(
      "compiled execution commit must stage only its one referenced run state",
    );
  }
  ...
  if (reviewConfig.enabled) {
    ...
    // On a first compilation the planning scope is added here. On a
    // recompilation it is already committed and may be byte-identical, so git
    // has nothing to stage. Either is fine; what matters is that the scope the
    // run state names actually exists under version control.
    const planningScopeStaged = entries.some(
      (entry) =>
        (entry.status === "A" || entry.status === "M") &&
        entry.path === planningTarget.scopeFile,
    );
    if (!planningScopeStaged) {
      requireCommittedContract(root, [planningTarget.scopeFile]);
    }
    if (
  ...
  // Every manifest for the plan must be under version control at this commit.
  // On a first compilation they are all staged. On a recompilation the
  // unchanged ones have nothing for git to stage, so an already-committed
  // manifest satisfies the requirement just as a staged one does.
  const unstaged = [...compiledPaths].filter(
    (path) => !manifestPaths.includes(path),
  );
  const uncommitted = unstaged.filter((path) => {
    try {
      requireCommittedContract(root, [path]);
      return false;
    } catch {
...
function validatePlanningReviewCommit(root, entries) {
  ...
  requireStagedMatchesWorktree(root, [scopeFile]);
  const manifest = parseManifestText(
    readFileSync(resolve(root, scopeFile), "utf8"),
  );
  if (manifest.state) requireStagedMatchesWorktree(root, [manifest.state]);
...
// The initialization sequence recognizes Product Plan, compiled execution,
// planning review and audited correction commits. It had no category for
// anything else, which meant the control plane could not be repaired and
// pre-existing repository content could not be adopted while initialization was
// in progress. Both are real needs and neither is application code.
//
// One rule covers both: every staged path is already a repository-contract path
// by isAdoptionContractPath, and nothing touches docs/plans. The outer gate
// enforces the first condition for every initialization commit; stating it here
// makes the classification explicit rather than implied. Excluding docs/plans is
// what guarantees such a commit can never alter an approved plan, a compiled
// manifest, run state or review evidence.
function validateRepositoryContractCommit(entries, installedArchitectureFiles) {
  if (!entries.length) return false;
  if (entries.some((entry) => entry.path.startsWith("docs/plans/"))) {
    return false;
  }
  return entries.every((entry) =>
    isAdoptionContractPath(entry.path, installedArchitectureFiles),
  );
}

export function validatePlanningCommitBoundary(root, config) {
  const entries = stagedEntries(root);
  requireStagedMatchesWorktree(
    root,
    entries
      .filter(
        (entry) => entry.status !== "D" && entry.path.startsWith("docs/plans/"),
      )
      .map((entry) => entry.path),
  );
  ...
  if (
    validateRepositoryContractCommit(
      entries,
      new Set(readArchitectureProfile(root).installedFiles),
    )
  ) {
...
function isAdoptionContractPath(path, installedArchitectureFiles) {
  const exact = new Set([
    // Repository contract files that predate initialization. These are the same
    // category as package.json and .node-version below: they configure the
    // repository rather than implement the product. Leaving them untracked is
    // not a neutral choice, because slice-worktree refuses to create a worker
    // worktree unless `git status --porcelain` is empty, so untracked files
    // block every worker from starting.
    ".gitignore",
    ".gitattributes",
    ".env.example",
    ".node-version",
  ...
  const prefixes = [
    // Pre-existing operator tooling adopted into the repository. It sits
    // outside the workspace, is excluded from the Turbo graph, lint, type
    // checking and build, and is therefore never inspected by any quality
    // gate. Requiring it to arrive through a gated slice would add no
    // verification and would leave it untracked in the meantime.
    "tools/",
    ".agents/",
    ".parallel-slices/",
    ".claude/",
    ...
    planningStage === "product-plan"
      ...
      : planningStage === "compiled-execution"
        ...
        : planningStage === "planning-review"
          ...
          : planningStage === "repository-contract"
            ? "Repository-contract commit gate passed"
...
function managedWorkerGoalRoot(root) {
  let goalRoot;
  try {
    goalRoot = primaryRepositoryRoot(root);
  } catch {
    return null;
  }
  const workerRoot = realpathSync(root);
  if (workerRoot === goalRoot) return null;
  const worktreesRoot = resolve(goalRoot, ".parallel-slices/runtime/worktrees");
  if (!existsSync(worktreesRoot)) return null;
  if (!workerRoot.startsWith(`${realpathSync(worktreesRoot)}${sep}`)) {
    return null;
  }
  return goalRoot;
}
...
function runConfiguredEntrypoint(root, config, entrypointId, options = {}) {
  ...
  let branch = options.branch || git(["branch", "--show-current"], root);
  let branchConfig = config;
  if (!branch) {
    const goalRoot = managedWorkerGoalRoot(root);
    if (goalRoot) {
      branch = git(["branch", "--show-current"], goalRoot);
      branchConfig = loadQualityConfig(goalRoot);
    }
  }
  assertEntrypointBranch(entrypointId, branch, branchConfig, options);
  ...
  } else if (planningStage === "repository-contract") {
    console.log("Repository-contract commit boundary passed");
  }

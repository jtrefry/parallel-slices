...
function discoverStatePath(root, requested) {
  if (requested) {
    ...
    return { statePath: requested, warnings: [] };
  }
  const lock = readJson(
  ...
  if (lock?.state) {
    ...
    return { statePath: lock.state, warnings: [] };
  }
  ...
  for (const name of readdirSync(directory)) {
    ...
    const path = `docs/plans/loop-runs/${name}`;
    try {
      candidates.push({ path, state: readRunState(root, path) });
    } catch (error) {
      warnings.push(
        `skipped unreadable run-state candidate ${path}: ${error.message}`,
      );
    }
  }
  ...
  if (active.length === 1) return { statePath: active[0].path, warnings };
  if (active.length > 1) {
    fail(
  ...
  if (candidates.length === 1)
    return { statePath: candidates[0].path, warnings };
  fail(
    warnings.length
      ? `no unique run state could be inferred; pass --state <path> (${warnings.join("; ")})`
      : "no unique run state could be inferred; pass --state <path>",
  );
}
...
function runtimeRecoveryWarnings(root, runId) {
  ...
  while (pending.length) {
    ...
    for (const entry of readdirSync(directory, { withFileTypes: true })) {
      ...
      if (entry.name.includes(".incoming-")) {
        warnings.push(
...
function runCli(argv) {
  ...
  const discovered = discoverStatePath(root, options.state);
  const summary = summarizeRunStatus(root, discovered.statePath);

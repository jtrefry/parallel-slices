...
When multi-agent review is enabled, the plan must survive independent review
...
all must approve. Reviewers look for what prose hides: a requirement whose
acceptance evidence no implementation could produce, a requirement with no
observable evidence or no possible owner, two requirements that contradict each
other, a conflict with the root instructions or the installed architecture, a
...
unsatisfiable requirement reads perfectly well, and the contradiction otherwise
surfaces only after compilation, after review of the compiled map, and after a
person has already spent their attention approving it. Machines check
satisfiability, traceability and contradiction; the human judges whether it is
the right product.

The review writes `docs/plans/reviews/<feature>/product-plan.json` and its
...
person approving knows the plan already survived independent scrutiny:
